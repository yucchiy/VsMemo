...
### Changed
- **Breaking Change**: Configuration file renamed from `types.json` to `config.json` for better clarity
- Automatic migration from `types.json` to `config.json` on first run

### Added
- Migration support for existing `types.json` files
  - Automatically detects and migrates old configuration files
  - Shows notification on successful migration
  - Cleans up old configuration file after migration

### Fixed
- Variables array is now properly preserved during configuration validation
- Improved test coverage for configuration service
